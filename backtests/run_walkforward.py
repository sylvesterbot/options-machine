...
def run_walkforward(start: dt.date, end: dt.date, out_path: str) -> pd.DataFrame:
    provider = MockSampleProvider()
...
def main() -> int:
    ...
    df = run_walkforward(start, end, args.out)
    print(f"walkforward done. trades={len(df)} out={args.out}")
    return 0
